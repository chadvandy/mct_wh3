...
function VLib.TriggerPopup(popup_key, text, use_two_buttons, button_one_callback, button_two_callback, creation_callback, opt_parent)
    -- verify shit is alright
    if not is_string(popup_key) then
        VLib.Error("trigger_popup() called, but the key passed is not a string!")
        return false
    end
    ...
    if not is_string(text) then
        VLib.Error("trigger_popup() called, but the text passed is not a string!")
        return false
    end
    ...
    if not is_boolean(use_two_buttons) then
        VLib.Error("trigger_popup() called, but the two_buttons arg passed is not a boolean!")
        return false
    end
